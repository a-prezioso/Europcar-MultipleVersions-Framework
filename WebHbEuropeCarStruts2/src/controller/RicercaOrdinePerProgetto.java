...
public class RicercaOrdinePerProgetto extends ActionSupport implements SessionAware {
	...
	public List<OrdineDiAcquistoDettaglio> findOrdinePerProgetto() {
		...
		Criteria criteria = dao.getCurrentSession().createCriteria(OrdineDiAcquistoDettaglio.class)
				.createAlias("oprogetto", "o").add(Restrictions.eq("o.idprogetto", Integer.parseInt(this.IDProgetto)));
		...
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>() {
			{
				...
				elencoProgetti = oPro.findAll();
				for (int i = 0; i < elencoProgetti.size(); i++) {
					put(String.valueOf(elencoProgetti.get(i).getIdprogetto()), elencoProgetti.get(i).getProgetto());
				}
			}
		};
	...
	public String avanti() {
		this.IDProgetto = (String) sessionMap.get(KEY_OGGETTOPROGETTO);
		return cerca();
	}

	public String cerca() {
		if (Integer.parseInt(this.IDProgetto) != 0) {
			sessionMap.put(KEY_OGGETTOPROGETTO, this.IDProgetto);
			AnnoContabile oAnnoContabile = (AnnoContabile) sessionMap.get(KEY_OGGETTOANNO);
			List<OrdineDiAcquistoDettaglio> elenco = this.findOrdinePerProgetto();
